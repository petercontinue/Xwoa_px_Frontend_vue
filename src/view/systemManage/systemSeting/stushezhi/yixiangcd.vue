...
import addyxchengduForm from "../../../../components/systemSetting/stusetting/addyxchengduForm";
import updateyxcdForm from "../../../../components/systemSetting/stusetting/updateyxcdForm";
export default {
  components: {
    addyxchengduForm,
    updateyxcdForm,
  ...
  methods: {
    ...mapActions(["GetAllYixiangchengduPages", "DeleteYixiangchengdu"]),
    ...
    getteachSubjectPages() {
      ...
      this.GetAllYixiangchengduPages({
        size,
        current,
      }).then((res) => {
    ...
    deleteTeachsubject() {
      ...
      if (this.checkList.length == 0) {
        ...
        this.$Modal.confirm({
          title: "提示",
          content: "<p>确定删除意向程度？</p>",
          onOk: () => {
            this.editdataID = this.checkList.id;
            this.DeleteYixiangchengdu({ Id: this.editdataID }).then((res) => {
              if (res.code == "Y" && res.success == true) {
