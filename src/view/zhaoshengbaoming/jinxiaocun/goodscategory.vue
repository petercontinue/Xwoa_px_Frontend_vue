...
<script>
import goodscategoryForm from "../../../components/zhaoshengbaoming/baomingjiaofei/goodscategoryForm";
...
export default {
  ...
  methods: {
    ...
    getteachSubjectPages() {
      api.get("/xwcloud-zsbm/zsbm/JinXiaoCun/GetAllTypeByPages",{
        size: this.size,
    ...
    deleteTeachsubject() {
      ...
      if (this.editdataID == "") {
        ...
        this.$Modal.confirm({
          ...
          content: "<p>确定删除该数据？</p>",
          onOk: () => {
            api.del("/xwcloud-zsbm/zsbm/JinXiaoCun/DeleteTeachingSuppliesType",{ ID: this.editdataID }).then(
              (res) => {
                if (res.code == "Y" && res.success == true) {
