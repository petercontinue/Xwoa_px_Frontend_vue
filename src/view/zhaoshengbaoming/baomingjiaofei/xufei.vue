<template>
  <div>
    <Row>
      <Alert show-icon style="margin-top: 0px">
        ...
        <Col style="margin-top: 10px; color: red">
          <span>4 使用打印功能时，请先选中要打印的数据行，在开始打印。</span>
        </Col>
      </Alert>
    </Row>
    ...
    <buttons-form
      v-model="menuID"
      v-on:handleSearch="getxufeiqiandanInfoPages"
    ...
    <daifukuanqiandan v-model="showdaifukuan" :moneystyle="2"></daifukuanqiandan>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    search(type) {
      ...
      this.getxufeiqiandanInfoPages();
    },

    getxufeiqiandanInfoPages() {
      api
        .get("/xwcloud-zsbm/zsbm/BaoMingJiaoFei/getAllXufeiQianDanInfoPages", {
          size: this.size,
    ...
    DeleteQianDanAndStuInfo() {
      ...
      if (this.editdataID == "") {
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            api
              .del("/xwcloud-zsbm/zsbm/BaoMingJiaoFei/DeleteQianDanAndStu", {
                qiandanID: this.editdataID,
              })
              .then((res) => {
                if (res.code == "Y" && res.success == true) {
      ...
      if (this.checkOne.length == 0) {
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            api
              .post("/xwcloud-zsbm/zsbm/BaoMingJiaoFei/updateqdbeizhu", {
                id: this.checkOne.id,
                beizhu: this.value,
              })
              .then((res) => {
