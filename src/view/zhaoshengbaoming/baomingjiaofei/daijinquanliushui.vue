  ...
  <div>
    <Row>
      <Alert show-icon style="margin-top: 0px">
        ...
        <Col>
          <span>温馨提示：该页面是学员报名（新签或续费）时使用代金券的流水记录；</span>
        </Col>
      </Alert>
    </Row>
...
export default {
        ...
        },
        {
          ...
          render: (h, params) => {
            if (params.row.banzhuren) {
              return h("div", params.row.banzhuren);
            } else {
              return h("div", "-");
            }
          },
  ...
  methods: {
    getteachSubjectPages() {
      api.get("/xwcloud-zsbm/zsbm/BaoMingJiaoFei/GetAllDaijinquanLiushuiPages",{
        size: this.size,
    ...
    deriveExcel(data) {
      axios
        .request({
          method: "get",
          url: "/xwcloud-zsbm/zsbm/BaoMingJiaoFei/exportDaijinquanliushui",
          headers: {
        ...
        .then((res) => {
          ...
          link.setAttribute("download", "代金券.xls");
          link.click();
          link = null;
