 ...
 * {min:10} 分
 * 根居时间字符串计算时间差，
 * @param startTime 2020-09-15 15:53:04 时间格式
 * @param endTime 2020-09-15 15:53:04 时间格式
 */
export const getTimeDifference = (startTime, endTime) => {
  ...
  if (startTime != null) {
    ...
    if (new Date(sd).getTime() > new Date(se).getTime()) {
      return "开始时间不允许大于结束时间"
    }
    let sde = new Date(se).getTime() - new Date(sd).getTime()  // 将时间字符转成时间戳
    if (sde > 1) {  // 当有差值时，就计算
      let m = sde % nd % nh / nm;
      let h = (sde % nd / nh + '').split('.')[0]
      if (h >= 1) {
        timeData['min'] = (Number(h) * 60 + m)
      } else {
        timeData['min'] = m
      }
    }
