  ...
  >
    <div>
      <Alert show-icon>
        ...
        <Col>
          <span>温馨提示：先【下载固定资产导入模板】Excel，组织好数据,然后再导入</span>
        </Col>
      </Alert>
    </div>
    ...
    <Row>
      <Col span="2">下载模板：</Col>
      <Col span="4">
        <Button type="success" @click="deMuban()">下载固定资产导入模板</Button>
      </Col>
...
export default {
  ...
  methods: {
    ...
    deMuban() {
      ...
      axios
        .request({
          method: "get",
          url: "/xwcloud-sys/sys/pxassets/ExportzichanMuban", //**** */
          headers: {
            Authorization: "Bearer " + getToken(),
          },
          params: {},
        ...
        .then((res) => {
          ...
          link.setAttribute("download", "导入固定资产模板.xls");
          link.click();
          link = null;
  ...
  mounted() {
    ...
    this.actionUrl = baseUrl + "xwcloud-sys/sys/pxassets/zichangExcel"; //****** */
