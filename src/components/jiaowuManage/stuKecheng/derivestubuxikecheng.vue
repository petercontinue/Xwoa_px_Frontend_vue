  ...
  >
    <Form ref="logForm" :model="logForm" :rules="logFormRule">
      ...
      <FormItem label="年级" prop="GradeID">
        <Select v-model="logForm.GradeID" placeholder="请选择年级">
          <Option v-for="item in allGradeData" :value="item.id" :key="item.id">{{
            item.name
          }}</Option>
        </Select>
      </FormItem>
  ...
  methods: {
    ok() {
      ...
      axios
        .request({
          method: "get",
          url: "/xwcloud-stu/stu/buxikecheng/ExportbuxiCourse",
          headers: {
            Authorization: "Bearer " + getToken(),
          },
          params: {
            campusID: this.logForm.campusID,
            GradeID: this.logForm.GradeID,
        ...
        .then((res) => {
          ...
          link.setAttribute("download", "学员课程导出.xls");
          link.click();
          link = null;
