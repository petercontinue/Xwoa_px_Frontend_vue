<template>
  <Modal
    v-model="show"
    title="导出学员生日"
    @on-ok="ok"
    :mask-closable="false"
    @on-cancel="closeModal(false)"
  >
    <Form ref="logForm" :model="logForm" :rules="logFormRule">
      <FormItem label="校区" prop="campusID">
        <Select v-model="logForm.campusID" placeholder="请选择校区">
          <Option v-for="item in allcampusData" :value="item.id" :key="item.id">{{
            item.name
          }}</Option>
        </Select>
      </FormItem>
      <FormItem label="年级" prop="stuGradeID">
        <Select v-model="logForm.stuGradeID" placeholder="请选择年级">
          <Option v-for="item in stugrade" :value="item.id" :key="item.id">{{
            item.name
          }}</Option>
        </Select>
      </FormItem>
      <FormItem label="日期" prop="joinTime">
        <DatePicker
          type="daterange"
          v-model="logForm.joinTime"
          placeholder="选择起止日期"
          style="width: 100%"
        ></DatePicker>
      </FormItem>
    </Form>
  </Modal>
</template>
<script>
import { mapActions } from "vuex";
import axios from "@/libs/api.request";
import { getToken } from "@/libs/util";
export default {
  name: "deriveautoxiake",
  props: {
    value: {
      type: Boolean,
      default: false,
    },
  },
  data() {
    return {
      show: this.value,
      logForm: {
        campusID: "",
        stuGradeID: "",
        joinTime: "",
      },
      logFormRule: {},
      allcampusData: [],
      stugrade: [], //学员年级
    };
  },
  methods: {
    ...mapActions(["getallcampusList", "getstugradeList"]),
    ok() {
      this.btnLoading = true;
      axios
        .request({
          method: "get",
          url: "/xwcloud-stu/stu/stuManagement/ExportStuBirth",
          headers: {
            Authorization: "Bearer " + getToken(),
          },
          params: {
            stuGradeID: this.logForm.stuGradeID,
            campusID: this.logForm.campusID,
            startDate: this.logForm.joinTime[0],
            endDate: this.logForm.joinTime[1],
          },
          responseType: "blob",
        })
        .then((res) => {
          this.btnLoading = false;
          // 文件下载
          const blob = new Blob([res], {
            type: "application/vnd.ms-excel",
          });
          let link = document.createElement("a");
          link.href = URL.createObjectURL(blob);
          link.setAttribute("download", "学员生日导出.xls");
          link.click();
          link = null;
          this.$Message.success("导出成功");
          this.closeModal(false);
        })
        .catch((err) => {
          this.btnLoading = false;
          this.$Message.error("下载失败");
        });
    },

    closeModal(val) {
      this.$emit("input", val);
    },
    getstugrade() {
      this.getstugradeList({}).then((res) => {
        if (res.code == "Y" && res.success == true) {
          this.stugrade = res.obj;
        }
      });
    },
  },
  watch: {
    value(val) {
      this.show = val;
    },
    show(val) {
      //当重新显示增加数据的时候重置整个form表单
      if (val) {
        this.$refs["logForm"].resetFields();
        this.getallcampusList({}).then((res) => {
          if (res.code == "Y" && res.success == true) {
            this.allcampusData = res.obj;
          }
        });
        this.getstugrade();
      } else {
        // 反之则关闭页面
        this.closeModal(val);
      }
    },
  },
};
</script>
