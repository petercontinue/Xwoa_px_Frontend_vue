<template>
  ...
  >
    <Form
      ...
      :label-colon="true"
    >
      <FormItem label="学员姓名（可多选）" prop="stuIDs">
        <Select
          v-model="wxqfForm.stuIDs"
          placeholder="请选择学员姓名（可多选）"
          multiple
          :filterable="true"
        >
    ...
    ok() {
      this.$refs["wxqfForm"].validate((valid) => {
        if (valid) {
          ...
          api
            .post("xwcloud-homeschool/homeschool/pxtuisongtable/wechatGroupSending", {
              stuIDs: stuidstring,
  ...
  watch: {
    ...
    show(val) {
      ...
      if (val) {
        ...
        api
          .get("/xwcloud-zsbm/zsbm/BaoMingJiaoFei/GetAllxufeistuList", { menuID: 271 })
          .then((res) => {
            if (res.code == "Y" && res.success == true) {
