...
import daochuKemuYuE from "@/components/shujutongji/daochuKemuYuE";
import * as api from "@/api/api.js";

export default {
  name: "subjectYuETongji",
  components: {
    daochuKemuYuE,
  },
  data() {
    return {
      columns6: [
        ...
        },
        {
          ...
          key: "moneyCount",
          align: "center",
          render: function (h, {row:{moneydate,moneynodate}}) {
            // console.log(moneydate,moneynodate)
            return h("div",parseFloat(moneydate)+parseFloat(moneynodate))
          }
        },
      ],
      data: [], // 接口数据接收
  ...
  methods: {
    getSubjectYuETongjiList() {
      api.get("xwcloud-caiwu/shujutongji/other/getSubjectYuETongji",this.params).then((res) => {
        if (res.code == "success") {
