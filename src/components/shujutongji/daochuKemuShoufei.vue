    ...
    <Form ref="formRef" :model="formData" :label-width="80">
      ...
      <FormItem label="科目" prop="subjectID">
        <Select v-model="formData.subjectID">
          <Option v-for="item in subjectList" :value="item.id" :key="item.id">{{
            item.name
          }}</Option>
        </Select>
      </FormItem>
...
export default {
  ...
  methods: {
    ...
    ok() {
      ...
      toolbox.exportExcel(
        "/xwcloud-caiwu/shujutongji/other/exportKemuShoufeiTongji",
        { campusID: this.formData.campusID,subjectID:this.formData.subjectID, datesoe: datesoe },
        "导出科目收费统计",
        this
      );
    },
